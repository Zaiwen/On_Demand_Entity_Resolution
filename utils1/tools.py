...
def svm_test(X, y, test_sizes=(0.2, 0.4, 0.6, 0.8), repeat=10):
    ...
    random_states = [18 + i for i in range(repeat)]
    ...
    for test_size in test_sizes:
        ...
        for i in range(repeat):
            ...
            if len(unique_classes) < 2:
                raise ValueError(
                    f"The training data needs samples of at least 2 classes, but it contains only {len(unique_classes)} class(es): {unique_classes}")
